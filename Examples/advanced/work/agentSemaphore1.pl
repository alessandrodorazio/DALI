...
eve(richiesta_attraversamento(var_Id,var_Priorita)):-now(var_Timestamp),assertz(vettura_prenotata(var_Id,var_Priorita,var_Timestamp)),format('La macchina con id ~w ha chiesto di attraversare al tempo ~2f',[var_Id,var_Timestamp]),nl.

a(elabora_coda):-write('Elaboro la coda...'),nl,max_member(var_B,vettura_prenotata(var__,var_B,var__)),write(var_B),vettura_prenotata(var_A,var_B,var_C),format('Macchina successiva: ~d',var_Id),nl,retract(strada_libera(true)),assertz(strada_libera(false)),a(concedi_autorizzazione(var_A)).
...
eve(termine_attraversamento(var_Id)):-retract(vettura_prenotata(var_Id,var__,var__)),format('La macchina con id ~d mi ha comunicato di aver terminato di attraversare',var_Id),nl,retract(strada_libera(false)),assertz(strada_libera(true)).

strada_libera:-write('').

evi(strada_libera):-strada_libera(var_X),true==var_X,vettura_prenotata(var_Id,var_Priorita,var_Timestamp),a(elabora_coda).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_351665,var_Ontology,_351669),_351659),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_351703)),a(message(var_Ag,propose(var_A,[_351703],var_AgI))),retractall(ext_agent(var_Ag,_351741,var_Ontology,_351745)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_351539,var_Ontology,_351543),_351533),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_351609,var_Ontology,_351613)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_351427,var_Ontology,_351431),_351421),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_351483,var_Ontology,_351487)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_350991),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_351025),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_350807).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_350655),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_350603),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_350479,_350481)),clause(agente(_350501,_350503,_350505,var_S),_350497),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_350273,_350275,_350277,var_S),_350269),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_350373,[]),repeat,read(_350373,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_350373).
